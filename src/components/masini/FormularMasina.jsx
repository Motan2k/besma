...
import { Modal, Field, Input, Select, Textarea, Row, Divider, FormActions, ErrorBox, SuccessBox } from '../common/index.jsx';
import { useMasiniActions, useLocatii, useProfiles } from '../../hooks/useData';
...
export default function FormularMasina({ isOpen, onClose, masina = null, onSuccess }) {
  ...
  // Găsește ID-ul Bucureștiului din lista de locații
  ...
  useEffect(() => {
    ...
    if (masina) {
      setForm({
        marca: masina.marca || '', model: masina.model || '',
        an_fabricatie: masina.an_fabricatie || new Date().getFullYear(),
        culoare: masina.culoare || '', combustibil: masina.combustibil || 'Benzină',
        nr_inmatriculare: masina.nr_inmatriculare || '', vin: masina.vin || '',
        km_actuali: masina.km_actuali || 0,
        locatie_id: masina.locatie_id || idBucuresti,
        sofer_id: masina.sofer_id || '',
        gps_imei: masina.gps_imei || '', note: masina.note || '', status: masina.status || 'activa',
      });
    } else {
      setForm({ ...initForm(), locatie_id: idBucuresti });
    }
    setError(''); setSuccess('');
  }, [masina, isOpen, idBucuresti]);

  // Actualizează locația default când se încarcă locațiile
  useEffect(() => {
    if (idBucuresti && !masina) {
      setForm(f => ({ ...f, locatie_id: idBucuresti }));
    }
  }, [idBucuresti, masina]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError('');
    if (!form.marca || !form.model || !form.nr_inmatriculare) {
    ...
    const { error } = isEdit ? await updateMasina(masina.id, payload) : await adaugaMasina(payload);
    setLoading(false);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEdit ? 'Editează mașina' : 'Adaugă mașină nouă'} width={620}>
      <form onSubmit={handleSubmit}>
        ...
        <Row cols={2}>
          <Field label="Marcă" required><Input placeholder="ex: Dacia" value={form.marca} onChange={set('marca')} /></Field>
          <Field label="Model" required><Input placeholder="ex: Logan" value={form.model} onChange={set('model')} /></Field>
        </Row>
        <Row cols={3}>
          ...
          <Field label="Culoare"><Input placeholder="ex: Alb" value={form.culoare} onChange={set('culoare')} /></Field>
        ...
        <Row cols={2}>
          <Field label="Nr. înmatriculare" required>
            <Input placeholder="ex: B 123 ABC" value={form.nr_inmatriculare}
              onChange={e => setForm(f => ({ ...f, nr_inmatriculare: e.target.value.toUpperCase() }))} />
          </Field>
          <Field label="Serie VIN">
            <Input placeholder="ex: UU1LSDB256..." value={form.vin}
